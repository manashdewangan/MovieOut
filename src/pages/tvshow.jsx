...
import { Link } from "react-router-dom"; // Ensure Link from react-router-dom is imported
...
const TvShows = () => {
  ...
  const [hoveredTvShowId, setHoveredTvShowId] = useState(null);
  const [loading, setLoading] = useState(false);
  const [hasSearched, setHasSearched] = useState(false);

  const apiKey = import.meta.env.VITE_API_TMDB; // Ensure you have your API key in the .env file

  // Fetch Popular TV Shows
  const fetchPopularTvShows = async (pageNumber = 1) => {
    setLoading(true);
    const url = `https://api.themoviedb.org/3/tv/popular?api_key=${apiKey}&page=${pageNumber}`;
    try {
      const response = await fetch(url);
      const data = await response.json();
      if (response.ok) {
        setPopularTvShows((prevTvShows) => [...prevTvShows, ...data.results]);
      } else {
        console.error("Error fetching popular TV shows:", data);
      }
    } catch (error) {
  ...
  const handleSearchSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error searching TV shows:", data);
      }
    } catch (error) {
  ...
  const tvShowsToDisplay = hasSearched ? searchResults : popularTvShows;
            ...
            {tvShowsToDisplay.length > 0 ? (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                  {tvShowsToDisplay.map((tvShow) => (
                    <Link key={tvShow.id} to={`/series/${tvShow.id}`}>
                      <div
                        className="relative bg-gray-900 rounded-lg overflow-hidden shadow-md transition transform hover:scale-105"
                        onMouseEnter={() => setHoveredTvShowId(tvShow.id)}
                        onMouseLeave={() => setHoveredTvShowId(null)}
                      >
                        <img
                          src={
                            tvShow.poster_path
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-400">
                  Sorry, no TV shows match your search. Try another keyword.
                </p>
              </div>
            ) : null}
          </>
